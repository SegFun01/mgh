...
curvas=[] # vector con las curvas de demanda / produccion
#---------->>>>>>>>>> Funcion para leer desde json
def leer_json(fin):
    global nn,nt,e,q,fi,h,de,a,l,d,ks,km,es,op,dia,hora,dow,mes,duracion
    ...
    d_curvas=j_red.get('curvas')    # se leen las curvas de demanda en una matriz
    for i in d_curvas.keys(): 
      curvas.append(d_curvas.get(i))
...
for i in range(duracion):                      # ejecutar la corrida estas veces
   print(f"hora:{hora} dia:{dia} dow:{dow}")   # dia del mes no estoy seguro de necesitarlo
   hora = (hora + 1) % 24
   if hora==0:
      dow = (dow +1) % 7
      dia = (dia + 1) % 30    # por conveniencia todos los meses se consideran de 30 dias, luego lo arreglo
yy=input("pausa...")      
